...
namespace WpfSimpleCalculator
{
    ...
    {
        public static void ReplaceLastWith<T>(this Queue<T> q, T element)
        {
        ...
        public static T Undo<T>(this Queue<T> q)
        {
            for (var i = 1; i < q.Count; i++)
                q.Enqueue(q.Dequeue());
        ...
        /// <returns>Item removed from the <see cref="System.Collections.Generic.Queue{T}"/>.</returns>
        public static IEnumerable<T> Undo<T>(this Queue<T> q, int quantity)
        {
